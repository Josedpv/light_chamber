...
//import { KTX2Loader } from './needed/KTX2Loader.js';s
...
// dracoLoader.preload();
//Scene and render
var  scene, bgScene, camera, cameraControls;
var bgMesh;
var engine;
var controls;
...
function addLights() 
{
	for (let index = 1; index < total; index++) {
		...
		plight[index]=new THREE.PointLight(0xffffff,0.1);
	...
	//plight[0]=new THREE.HemisphereLight(0xffffff,1);
	//scene.add(plight[0]);
}
//******************************************************************************************fireflies Tween
function addGUI() 
...
for (let index = 1; index < total; index++) {
	...
	cube[index]	= new THREE.Mesh( new THREE.SphereGeometry( 0.5, 10, 10 ), new THREE.MeshBasicMaterial() );
	
	scene.add( cube[index] );

	cube2[index]	= new THREE.Mesh( new THREE.SphereGeometry( 0.7, 10, 10 ), new THREE.MeshBasicMaterial({ color: "white", transparent:true, opacity:0.50 }) );
...
addLights();
/*********************************************************************************************************** */	
loadGLTF('../needed/sponza-ktx.glb', [1, 0, 0], [5, 5, 5]).then(function(gltf){
...
function loadGLTF(path, pos,scale) {
	...
	//	var loader = new GLTFLoader().setCrossOrigin('anonymous').setDRACOLoader(new DRACOLoader().setDecoderPath('https://www.gstatic.com/draco/v1/decoders/')).setKTX2Loader(new KTX2Loader());
		...
		//loader.setDRACOLoader( dracoLoader ).setCrossOrigin('anonymous').setKTX2Loader(new KTX2Loader(  ) );
